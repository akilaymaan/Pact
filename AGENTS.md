...
- Visual reference: `DESIGN.md`. The page uses cobalt, warm paper, and charcoal section palettes with Geist Pixel Square display type, DM Sans body type, Geist Mono metadata, and Arial buttons.
- Fonts are self-hosted in `public/fonts/`. The Geist files come from the official `geist@1.7.0` distribution; DM Sans is the Google Fonts variable build. Both are under the SIL Open Font License — keep the included license with the font assets. No Next.js font loader or external font service is needed.
